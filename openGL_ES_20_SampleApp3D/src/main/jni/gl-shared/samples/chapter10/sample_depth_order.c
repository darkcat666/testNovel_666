...
} Extension_DepthOrder;

/**
 * アプリの初期化を行う
 */
void sample_DepthOrder_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_DepthOrder*) malloc(sizeof(Extension_DepthOrder));
    // サンプルアプリ用データを取り出す
    Extension_DepthOrder *extension = (Extension_DepthOrder*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_DepthOrder_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_DepthOrder_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_DepthOrder *extension = (Extension_DepthOrder*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_DepthOrder_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_DepthOrder *extension = (Extension_DepthOrder*) app->extension;
