 ...
 * アプリの初期化を行う
 */
void sample_BlendOrder_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_BlendOrder*) malloc(sizeof(Extension_BlendOrder));
    // サンプルアプリ用データを取り出す
    Extension_BlendOrder *extension = (Extension_BlendOrder*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_BlendOrder_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_BlendOrder_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_BlendOrder *extension = (Extension_BlendOrder*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_BlendOrder_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_BlendOrder *extension = (Extension_BlendOrder*) app->extension;
