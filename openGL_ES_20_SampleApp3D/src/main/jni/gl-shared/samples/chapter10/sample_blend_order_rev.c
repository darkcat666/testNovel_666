...
} Extension_BlendOrderRev;

/**
 * アプリの初期化を行う
 */
void sample_BlendOrderRev_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_BlendOrderRev*) malloc(sizeof(Extension_BlendOrderRev));
    // サンプルアプリ用データを取り出す
    Extension_BlendOrderRev *extension = (Extension_BlendOrderRev*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_BlendOrderRev_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_BlendOrderRev_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_BlendOrderRev *extension = (Extension_BlendOrderRev*) app->extension;
    ...
    // 四角形を描画する
    {
        // 描画色を指定する
        // R, G, B, A
        glUniform4f(extension->unif_color, 1.0f, 0.0f, 1.0f, 0.5f);
    ...
    // 三角形を描画する
    {
        // 描画色を指定する
        // R, G, B, A
        glUniform4f(extension->unif_color, 1.0f, 1.0f, 1.0f, 0.5f);
 ...
 * アプリのデータ削除を行う
 */
void sample_BlendOrderRev_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_BlendOrderRev *extension = (Extension_BlendOrderRev*) app->extension;
