...
} Extension_DepthOrderRev;

/**
 * アプリの初期化を行う
 */
void sample_DepthOrderRev_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_DepthOrderRev*) malloc(sizeof(Extension_DepthOrderRev));
    // サンプルアプリ用データを取り出す
    Extension_DepthOrderRev *extension = (Extension_DepthOrderRev*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_DepthOrderRev_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_DepthOrderRev_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_DepthOrderRev *extension = (Extension_DepthOrderRev*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_DepthOrderRev_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_DepthOrderRev *extension = (Extension_DepthOrderRev*) app->extension;
