 ...
 * アプリの初期化を行う
 */
void sample_PmdLoad_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_PmdLoad*) malloc(sizeof(Extension_PmdLoad));
    // サンプルアプリ用データを取り出す
    Extension_PmdLoad *extension = (Extension_PmdLoad*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_PmdLoad_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_PmdLoad_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdLoad *extension = (Extension_PmdLoad*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_PmdLoad_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdLoad *extension = (Extension_PmdLoad*) app->extension;
