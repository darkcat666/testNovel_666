...
typedef struct {
    ...
    // サンプルの実行開始時間
    time_t startTime;
} Extension_PmdMultirenderVBO;

/**
 * アプリの初期化を行う
 */
void sample_PmdMultirenderVBO_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_PmdMultirenderVBO*) malloc(sizeof(Extension_PmdMultirenderVBO));
    // サンプルアプリ用データを取り出す
    Extension_PmdMultirenderVBO *extension = (Extension_PmdMultirenderVBO*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_PmdMultirenderVBO_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * PMDファイルのレンダリングを行う
 */
void sample_PmdMultirenderVBO_renderingPMD(Extension_PmdMultirenderVBO *extension, const mat4 wlpMatrix) {
    // バッファオブジェクトのバインドを行う
    glBindBuffer(GL_ARRAY_BUFFER, extension->vertices_buffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, extension->indices_buffer);
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_PmdMultirenderVBO_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdMultirenderVBO *extension = (Extension_PmdMultirenderVBO*) app->extension;
    ...
    // レンダリング負荷を掛けるために大量のモデルを描画する
    {
        ...
        for (x = 0; x < xModels; ++x) {
            for (z = 0; z < zModels; ++z) {
                ...
                sample_PmdMultirenderVBO_renderingPMD(extension, mat4_multiply(lp, world));
            }
        }
 ...
 * アプリのデータ削除を行う
 */
void sample_PmdMultirenderVBO_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdMultirenderVBO *extension = (Extension_PmdMultirenderVBO*) app->extension;
