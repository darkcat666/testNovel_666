...
typedef struct {
    ...
    // モデルの透過
    GLfloat alpha;
} Extension_RenderAlpha;

/**
 * アプリの初期化を行う
 */
void sample_RenderAlpha_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_RenderAlpha*) malloc(sizeof(Extension_RenderAlpha));
    // サンプルアプリ用データを取り出す
    Extension_RenderAlpha *extension = (Extension_RenderAlpha*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_RenderAlpha_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_RenderAlpha_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_RenderAlpha *extension = (Extension_RenderAlpha*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_RenderAlpha_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_RenderAlpha *extension = (Extension_RenderAlpha*) app->extension;
