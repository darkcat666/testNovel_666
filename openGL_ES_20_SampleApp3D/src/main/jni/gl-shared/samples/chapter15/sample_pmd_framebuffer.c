...
} Extension_PmdFramebuffer;

/**
 * アプリの初期化を行う
 */
void sample_PmdFramebuffer_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_PmdFramebuffer*) malloc(sizeof(Extension_PmdFramebuffer));
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebuffer *extension = (Extension_PmdFramebuffer*) app->extension;
    ...
    // レンダリングターゲットを用意する
    {
        ...
        // テクスチャとバッファをフレームバッファへアタッチする
        {
            ...
            // フレームバッファをデフォルトに戻す
            // Androidは0だが、iOSはデフォルトのフレームバッファへ戻さなければならない
 ...
 * レンダリングエリアが変更された
 */
void sample_PmdFramebuffer_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * PMDファイルのレンダリングを行う
 */
void sample_PmdFramebuffer_renderingPMD(Extension_PmdFramebuffer *extension, const mat4 wlpMatrix) {
    // バッファオブジェクトのバインドを行う
    glBindBuffer(GL_ARRAY_BUFFER, extension->vertices_buffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, extension->indices_buffer);
 ...
 * 立方体を描画する
 */
void sample_PmdFramebuffer_renderingCube(GLApplication *app, GLuint texture) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebuffer *extension = (Extension_PmdFramebuffer*) app->extension;
 ...
 * 2D描画を行う
 */
void sample_PmdFramebuffer_renderingSprite(GLApplication *app, GLuint texture, int x, int y, int width, int height) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebuffer *extension = (Extension_PmdFramebuffer*) app->extension;
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_PmdFramebuffer_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebuffer *extension = (Extension_PmdFramebuffer*) app->extension;
    ...
    // フレームバッファへ一度レンダリングする
    {
        ...
        sample_PmdFramebuffer_renderingPMD(extension, wlpMatrix);
    ...
    // 立方体として描画する
    {
        sample_PmdFramebuffer_renderingCube(app, extension->target.colorTexture);
    }

    // 2Dとして描画する
    {
        sample_PmdFramebuffer_renderingSprite(app, extension->target.colorTexture, 10, 10, extension->target.width, extension->target.height);
    }
 ...
 * アプリのデータ削除を行う
 */
void sample_PmdFramebuffer_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebuffer *extension = (Extension_PmdFramebuffer*) app->extension;
