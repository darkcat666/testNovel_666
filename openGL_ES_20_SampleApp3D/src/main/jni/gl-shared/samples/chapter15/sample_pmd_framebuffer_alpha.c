...
} Extension_PmdFramebufferAlpha;

/**
 * アプリの初期化を行う
 */
void sample_PmdFramebufferAlpha_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_PmdFramebufferAlpha*) malloc(sizeof(Extension_PmdFramebufferAlpha));
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebufferAlpha *extension = (Extension_PmdFramebufferAlpha*) app->extension;
    ...
    // レンダリングターゲットを用意する
    {
        ...
        // 描画対象のテクスチャを生成する
        {
            glGenTextures(1, &extension->target.colorTexture);
            assert(glGetError() == GL_NO_ERROR);
            assert(extension->target.colorTexture != 0);

            // テクスチャのメモリを確保する
            glBindTexture(GL_TEXTURE_2D, extension->target.colorTexture);
 ...
 * レンダリングエリアが変更された
 */
void sample_PmdFramebufferAlpha_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * PMDファイルのレンダリングを行う
 */
void sample_PmdFramebufferAlpha_renderingPMD(Extension_PmdFramebufferAlpha *extension, const mat4 wlpMatrix) {
 ...
 * 立方体を描画する
 */
void sample_PmdFramebufferAlpha_renderingCube(GLApplication *app, GLuint texture) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebufferAlpha *extension = (Extension_PmdFramebufferAlpha*) app->extension;
 ...
 * 2D描画を行う
 */
void sample_PmdFramebufferAlpha_renderingSprite(GLApplication *app, GLuint texture, int x, int y, int width, int height) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebufferAlpha *extension = (Extension_PmdFramebufferAlpha*) app->extension;
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_PmdFramebufferAlpha_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebufferAlpha *extension = (Extension_PmdFramebufferAlpha*) app->extension;
    ...
    // フレームバッファへ一度レンダリングする
    {
        ...
        sample_PmdFramebufferAlpha_renderingPMD(extension, wlpMatrix);
    ...
    // 立方体として描画する
    {
        ...
        sample_PmdFramebufferAlpha_renderingCube(app, extension->target.colorTexture);
    }

    // 2Dとして描画する
    {
        sample_PmdFramebufferAlpha_renderingSprite(app, extension->target.colorTexture, 10, 10, extension->target.width, extension->target.height);
    }
 ...
 * アプリのデータ削除を行う
 */
void sample_PmdFramebufferAlpha_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdFramebufferAlpha *extension = (Extension_PmdFramebufferAlpha*) app->extension;
