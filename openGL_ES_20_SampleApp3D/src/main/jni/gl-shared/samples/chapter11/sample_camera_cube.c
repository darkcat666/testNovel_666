...
typedef struct {
    ...
    // 回転角
    GLfloat rotate;
} Extension_CameraCube;

/**
 * アプリの初期化を行う
 */
void sample_CameraCube_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_CameraCube*) malloc(sizeof(Extension_CameraCube));
    // サンプルアプリ用データを取り出す
    Extension_CameraCube *extension = (Extension_CameraCube*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_CameraCube_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_CameraCube_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_CameraCube *extension = (Extension_CameraCube*) app->extension;
 ...
 * アプリのデータ削除を行う
 */
void sample_CameraCube_destroy(GLApplication *app) {
// サンプルアプリ用データを取り出す
    Extension_CameraCube *extension = (Extension_CameraCube*) app->extension;
