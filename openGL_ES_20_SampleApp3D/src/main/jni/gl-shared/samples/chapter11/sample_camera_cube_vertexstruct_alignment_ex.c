...
typedef struct {
    ...
    // 回転角
    GLfloat rotate;
} Extension_CameraCubeVertexstructAlignmentEx;

/**
 * アプリの初期化を行う
 */
void sample_CameraCubeVertexstructAlignmentEx_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_CameraCubeVertexstructAlignmentEx*) malloc(sizeof(Extension_CameraCubeVertexstructAlignmentEx));
    // サンプルアプリ用データを取り出す
    Extension_CameraCubeVertexstructAlignmentEx *extension = (Extension_CameraCubeVertexstructAlignmentEx*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_CameraCubeVertexstructAlignmentEx_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_CameraCubeVertexstructAlignmentEx_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_CameraCubeVertexstructAlignmentEx *extension = (Extension_CameraCubeVertexstructAlignmentEx*) app->extension;
    ...
    // キューブを構築する
    {
        ...
        // packedを設定することでAlignmentを回避するが、描画結果は保証されない。
        // 頂点構造体を定義
        ...
        // バイト境界が設定されていることを確認する
        assert(sizeof(CubeVertex) == (sizeof(vec3) + sizeof(GLbyte) + sizeof(GLbyte)));
        ...
        // UVを転送する
        glVertexAttribPointer(extension->attr_uv, 2, GL_BYTE, GL_FALSE, sizeof(CubeVertex), (GLvoid*) ((GLubyte*) cube + sizeof(vec3)));
 ...
 * アプリのデータ削除を行う
 */
void sample_CameraCubeVertexstructAlignmentEx_destroy(GLApplication *app) {
// サンプルアプリ用データを取り出す
    Extension_CameraCubeVertexstructAlignmentEx *extension = (Extension_CameraCubeVertexstructAlignmentEx*) app->extension;
