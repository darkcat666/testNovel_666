...
typedef struct {
    ...
    // サンプルの実行開始時間
    time_t startTime;
} Extension_PmdGlFinish;

/**
 * アプリの初期化を行う
 */
void sample_PmdGlFinish_initialize(GLApplication *app) {
    // サンプルアプリ用のメモリを確保する
    app->extension = (Extension_PmdGlFinish*) malloc(sizeof(Extension_PmdGlFinish));
    // サンプルアプリ用データを取り出す
    Extension_PmdGlFinish *extension = (Extension_PmdGlFinish*) app->extension;
 ...
 * レンダリングエリアが変更された
 */
void sample_PmdGlFinish_resized(GLApplication *app) {
    // 描画領域を設定する
 ...
 * PMDファイルのレンダリングを行う
 */
void sample_PmdGlFinish_renderingPMD(Extension_PmdGlFinish *extension, const mat4 wlpMatrix) {
    // バッファオブジェクトのバインドを行う
    glBindBuffer(GL_ARRAY_BUFFER, extension->vertices_buffer);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, extension->indices_buffer);
 ...
 * 毎秒60回前後呼び出される。
 */
void sample_PmdGlFinish_rendering(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdGlFinish *extension = (Extension_PmdGlFinish*) app->extension;
    ...
    // レンダリング負荷を掛けるために大量のモデルを描画する
    {
        ...
        for (x = 0; x < xModels; ++x) {
            for (z = 0; z < zModels; ++z) {
                ...
                sample_PmdGlFinish_renderingPMD(extension, mat4_multiply(lp, world));
            }
        }
 ...
 * アプリのデータ削除を行う
 */
void sample_PmdGlFinish_destroy(GLApplication *app) {
    // サンプルアプリ用データを取り出す
    Extension_PmdGlFinish *extension = (Extension_PmdGlFinish*) app->extension;
